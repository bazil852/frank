...
export default function CloseMatchCard({ product, reasons, improvements, index }: CloseMatchCardProps) {
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className="bg-amber-50  rounded-2xl p-6 shadow-sm border border-amber-200 dark:border-amber-800/50 hover:shadow-lg hover:border-amber-300 dark:hover:border-amber-700 transition-all duration-200 group"
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h3 className="font-bold text-slate-900  text-lg group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors">{product.provider}</h3>
              {product.interestRate && (
                <span className="bg-amber-100  text-amber-800 dark:text-amber-300 px-2 py-1 rounded-lg text-xs font-semibold">
                  {product.interestRate[0]}%-{product.interestRate[1]}%
                </span>
              )}
            </div>
            <p className="text-sm text-slate-500  font-medium mt-1 transition-colors">
              {product.productType} • R{(product.amountMin / 1000).toFixed(0)}k-R{(product.amountMax / 1000).toFixed(0)}k
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 bg-amber-100  px-3 py-1.5 rounded-full transition-colors">
          <Clock size={14} className="text-amber-600" />
          <span className="text-sm font-medium text-amber-700  transition-colors">{product.speedDays[0]}-{product.speedDays[1]} days</span>
        </div>
      </div>

      {reasons.length > 0 && (
        <div className="mb-5">
          <p className="text-sm font-semibold text-amber-800  mb-3 transition-colors">Current gaps:</p>
